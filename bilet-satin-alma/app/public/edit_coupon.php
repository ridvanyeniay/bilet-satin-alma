...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($code) || $discount_rate === false || $usage_limit === false || empty($valid_until)) {
        ...
        try {
            $db_discount_rate = $discount_rate / 100.0; // Oranı veritabanı formatına çevir (20 -> 0.20)
            ...
            ");
            $stmt->execute([$code, $db_discount_rate, $usage_limit, $valid_until, $coupon_id]);
...
<div class="page-container">
    <h1>Genel Kuponu Düzenle</h1>
    <p><a href="/admin_panel.php">&larr; Admin Paneline Geri Dön</a></p>

    <div class="admin-section">
        <form action="edit_coupon.php?id=<?= htmlspecialchars($coupon_id) ?>" method="POST" class="admin-form">
